...
export class AgendaMedicoRepository extends BaseRepository<AgendaMedico> implements IAgendaMedicoRepository {
  ...
  async findAll(page: number = 1, limit: number = 10, dataAgendamentoMin: Date, dataAgendamentoMax: Date): Promise<AgendaMedico[]> {
  ...
  if (dataAgendamentoMin) {
    query.andWhere("agendaMedico.dataAgendamento >= :dataAgendamentoMin", { dataAgendamentoMin });
  }

  if (dataAgendamentoMax) {
    query.andWhere("agendaMedico.dataAgendamento <= :dataAgendamentoMax", { dataAgendamentoMax });
  }
  ...
  async count(page: number = 1, limit: number = 10, dataAgendamentoMin: Date, dataAgendamentoMax: Date): Promise<number> {
    const query = this._repository
      .createQueryBuilder("agendaMedico")
      .leftJoinAndSelect("agendaMedico.medico", "medico")
      .leftJoinAndSelect("medico.usuario", "medicoUsuario")
  ...
  async update(id: number, agendaMedico: IAgendaMedico): Promise<AgendaMedico> {
    ...
    const agendaMedicoAtualizado = this._repository.merge(agendaMedicoExistente, agendaMedico);
    const updatedAgendaMedico = await this._repository.save(agendaMedicoAtualizado);
