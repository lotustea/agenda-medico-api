...
export class PessoaFisicaRepository
  extends BaseRepository<PessoaFisica>
  implements IPessoaFisicaRepository
{
  constructor() {
    super(PessoaFisica);
  }
  ...
  async update(id: number, pessoaFisica: IPessoaFisica): Promise<PessoaFisica> {
    await this._repository.update(id, pessoaFisica);
    const updatedPessoaFisica = await this._repository.findOne({
      where: { id },
    });
